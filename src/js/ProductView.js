...
class ProductView {
  ...
  createProductList(products) {
    ...
    if (products.length === 0)
      result += `<h2 class="font-bold animate-bounce text-center w-full text-xs sm:text-base md:text-lg lg:text-xl">No product to show</h2>`;
    products.forEach((p) => {
      result += `<div class="flex items-center justify-between drop-shadow-lg bg-gray-100 p-2 rounded-lg">
        ...
        <div class="flex items-center">
          ...
          <div class="ml-2 text-xs sm:ml-6 lg:text-xl sm:text-base md:text-lg">
            <span class="text-slate-500">${new Date(p.createdDate).toLocaleDateString(
              "en-GB"
            )}</span>
          </div>
          <!-- product category -->
